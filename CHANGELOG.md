...
### Fixed
- The conversation opencode is moving into its `session_message` table is read. opencode keeps a turn per row there, with the role in a `type` column and an assistant turn's parts inside one blob, and the old `message` and `part` tables go away with the switch. A store that has moved answered every query deja ran with `no such table: session` — the harness read as unreadable and none of its sessions were indexed — or, where the old tables are still there and empty, read as a store with no sessions at all, which nothing complains about. deja now asks where the turns are and reads the layout it finds, including the two renames inside a turn: a tool names itself under `$.name` rather than `$.tool`, and what a command printed is the block list `$.state.content` rather than the string `$.state.output`. Reported in #3924. (#3924)
