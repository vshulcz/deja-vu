...
### Added
- `deja blame` says when the line it was given is not a line. The answer learned to name which silence it is when it cannot answer for a line (#3726) and still said nothing when the spec itself was the problem: `:0`, a number past what an int holds, `:abc`. The file answer printed as if no line had been asked for, so a typed mistake read as "this file has no history". Found by sweeping eighteen forms against a three-line file the day the line answer landed. Two shapes, and they need different care — `:0` and the overflow are cut off by the suffix trim and simply vanished, while `:abc`, `:-1` and `:2.5` stay part of the path, so the sentence for those is printed only when nothing of that name is on disk: a file whose own name carries a colon is not a mistake to report. (#3738)
