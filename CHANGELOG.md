...
### Fixed
- opencode 2.x stores are read again. opencode 2.0 renamed `session` to `session_v2` and folded `message` and `part` into one `session_message` table, so every query came back `no such table: session`: the harness reported unreadable and none of its sessions reached the index, on a store that had 154 of them. Both schemas are read now, chosen by asking `sqlite_master`, with the two renames inside a turn handled — a tool names itself under `$.name` rather than `$.tool`, and what a command printed is the block list `$.state.content` rather than the string `$.state.output`. Reported with the live shapes by @yourfriendaaron in #3924, which is where the fixture came from. (#3924)
