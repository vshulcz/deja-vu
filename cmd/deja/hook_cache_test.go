...
func TestCachedHookDigestServesWithinTTLAndInvalidates(t *testing.T) {
	...
	d1, s1, _, _, _, _, _ := cachedHookDigest(dir)
	if d1 == "" || s1 == 0 {
	...
	d2, _, _, _, _, _, _ := cachedHookDigest(dir)
	if d2 != d1 {
	...
	d4, _, _, _, _, _, _ := cachedHookDigest(dir)
	if d4 == d3 && d3 != "" {
